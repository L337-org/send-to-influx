...
5. **MCP read metadata**: set `MCP_FIELD_METADATA` (units + `codes` for coded fields, from UNITS.md), and `MCP_MEASUREMENT`/`MCP_TAG_FILTERS` if the InfluxDB measurement isn't the source name or is shared - the read tools then expose the source automatically
