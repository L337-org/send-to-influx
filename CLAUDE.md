...
- `.github/workflows/release.yaml`: pushing a bare `MAJOR.MINOR` tag (e.g. `3.0` - matching this project's existing tags/releases, no `v` prefix) runs the test suite, verifies the tag matches `pyproject.toml`'s version exactly, builds the `.deb`, and attaches it to a GitHub Release. That's all it does: APT publishing moved out on 2026-07-15. The release step decides whether a release already exists (GitHub's "Draft a new release" UI creates tag+release together) from the *list* endpoint and then creates/uploads by release **id**, never the by-tag lookup endpoint, with retries on every call - hardened after the 4.3 release, when `releases/tags/{tag}` served 503s for an extended period while the list endpoint kept working, and `gh release upload <tag>` rendered the 503 as a bogus "release not found" that failed the run (the 4.3 `.deb` was rescued by hand-uploading the run's artifact via the release-id endpoint, the same path the workflow now uses).
