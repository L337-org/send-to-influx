...
If a write to InfluxDB fails, `send_data()` buffers the point in memory instead of dropping it (raising `InfluxWriteError` either way, so the worker's existing backoff/retry is unaffected) - see `DataHandler._write_buffers`: a per-source `deque(maxlen=MAX_BUFFERED_POINTS)`, class-level rather than an instance attribute because the worker loop in `sendtoinflux.py` discards and reconstructs the `DataHandler` instance after every failure, so only a buffer that outlives the instance survives to be flushed. The next successful `send_data()` call for that source flushes the backlog (oldest first) before sending the new point; if a source's outage runs long enough to fill its buffer, the oldest buffered point is dropped to make room, logged as a warning. Not persisted across a process restart - the existing backoff already tolerates fairly long outages before the worker itself would need restarting. `InfluxWriteError.bufferable` (default `True`) distinguishes transient failures (network errors, 5xx - worth buffering) from a `400` (InfluxDB rejected this exact point as malformed - retrying the identical payload would fail identically forever, so it's dropped rather than buffered, which would otherwise permanently block flushing every point behind it); `_flush_buffer()` drops just the offending buffered point on a non-bufferable failure and keeps flushing the rest, since that failure says nothing about whether InfluxDB itself is reachable.
