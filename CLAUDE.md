...
- `packaging/deb/preinst` removes any *version-named* `venv/lib/python3.X` directory left by the
  pre-4.4 venv layout, so dpkg can lay down the current one cleanly. Two traps are encoded here.
  First, `build-deb.sh` now always names the real site-packages directory `lib/python3` with every
  `lib/python3.X` a symlink to it: previously the real directory was named after whichever
  interpreter built the package, so a package built against a different Python minor than the
  installed one required a directory and a symlink to swap places - which dpkg cannot reliably do,
  producing ~166 "unable to delete old directory" warnings and potentially leaving the old directory
  shadowing the new one. Second, and more dangerous: **`dpkg-reconfigure` runs `preinst` as
  `upgrade <version>`, indistinguishable from a real upgrade, but with no unpack following it** - so
  anything `preinst` deletes on that path is gone permanently. An earlier version removed the whole
  venv and destroyed the installation on every reconfigure. The `-type d` match is what keeps it
  safe: it matches only real directories, never the symlinks of the current layout, so it is a no-op
  on an already-migrated install and safe to re-run.
