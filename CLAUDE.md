...
- The `mcp` SDK is imported only inside `toinflux/mcpserver.py` (lazily, gated on `mcp_enabled()`),
  ...
  requirements.txt (2026.x CalVer releases dropped Python 3.10 wheels) and `cryptography` to
  `~=48.0.1` (49.0.0 dropped the macOS universal2 wheel); the build fails loudly if coverage
  regresses.
